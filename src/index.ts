...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Wikidata SPARQL');
}
...
const tools: McpToolExport['tools'] = [
  {
    ...
    inputSchema: {
      type: 'object',
      properties: {
        sparql: { type: 'string', description: 'SPARQL query text. Also accepts `query` as an alias.' },
        query: { type: 'string', description: 'Alias for `sparql`.' },
        format: { type: 'string', description: 'json (default) | xml | csv | tsv | text/html' },
      },
      // Deliberately not `required: ['sparql']` — the gateway's pre-flight
      // guard rejects a call missing a declared required argument BEFORE this
      // pack ever runs, so a caller sending only the `query` alias would never
      // reach reqSparql() below to be told it's fine. The runtime check there
      // is what enforces "at least one of the two", same pattern as the
      // weather pack's location/latitude+longitude OR requirement.
    },
  },
  {
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    case 'query':
      return runSparql(reqSparql(args), (args.format as string) ?? 'json');
    case 'current_officeholder': {
      ...
      return {
        ...
        // Same value as `entity.name`, under a key the gateway's entity-check
        // detector actually reads (fleet #1846, phase 3b — the nested
        // `entity.name` isn't on its ENTITY_KEYS allowlist and a generic bare
        // `name` is deliberately not either, since it collides with row names
        // in list responses elsewhere). This answer is about the PLACE asked
        // for ("mayor of London" resolves London, not the mayor), so this is
        // the resolved place, never the office-holder's own name.
...
async function runSparql(query: string, format: string) {
  ...
  // Long queries MUST go by POST — the query cannot ride in the URL forever.
  //
  // This sent every query as `?query=<encoded>`, which has a hard ceiling: at
  // 300 `VALUES` pairs (~6.6KB encoded) WDQS answers **414 Request-URI Too
  // Large**, and somewhere below that the query arrives truncated instead,
  // which surfaces as `MalformedQuery` on a template that is perfectly valid.
  // Measured 2026-08-17: 5 / 50 / 150 pairs all fine, 300 pairs → 414; the
  // same 8.8KB query POSTed returns 200.
  //
  // That was 61 errors in 24h — a 33% error rate on this tool, with the most
  // recent hour 9 errors and zero successes — and every one of them looked like
  // a caller writing bad SPARQL. It was not. The queries were ours (60 of 61
  // from `internal:pipeworx`) and they were correct; the transport was
  // shortening them.
  //
  // GET is kept for short queries because WDQS and the CDN in front of it can
  // cache a GET and cannot cache a POST, and the overwhelming majority of
  // queries are small. The threshold is on the ENCODED length, since that is
  // what the URL limit actually applies to, and it sits well under the ~8KB
  // ceiling so there is room for the endpoint and headers.
  //
  // Deliberately NOT reclassifying `MalformedQuery` as `user_error` while this
  // was outstanding: that would have hidden this bug behind "the caller wrote
  // bad SPARQL", which is exactly the wrong direction when the caller is us.
  const encoded = encodeURIComponent(query);
  const res = encoded.length <= 4000
    ? await pwFetch(`${ENDPOINT}?query=${encoded}`, {
      headers: { Accept: accept, 'User-Agent': UA },
    })
    : await pwFetch(ENDPOINT, {
      method: 'POST',
      headers: { Accept: accept, 'User-Agent': UA, 'Content-Type': 'application/sparql-query' },
      body: query,
    });
  ...
  if (!res.ok) {
    const t = await res.text();
    throw new Error(`WDQS error: ${res.status} ${wdqsErrorDetail(t, query, encoded.length)}`);
  }
...
/**
 * Say what WDQS actually objected to, instead of the first 200 characters of it.
 *
 * A WDQS 4xx body is 9-13 KB in this exact order: `SPARQL-QUERY: queryStr=` and
 * a full echo of the query, THEN a Java stack trace, and only at the very
 * bottom the deepest `Caused by:` with the parser's real verdict. So
 * `body.slice(0, 200)` — what this did — kept the one part the caller already
 * had and threw away the one part they needed.
 *
 * It was worse than merely unhelpful. The 200 chars land partway through the
 * echoed query, so every error read as "WDQS received a query cut off
 * mid-token", and on 2026-08-17 that was taken as evidence the CALLER was
 * sending truncated SPARQL. It was not evidence of anything; the cut was ours,
 * and the measurable tell was that every error message was byte-for-byte 216
 * characters long. Two rounds of diagnosis went at the wrong layer. (fleet #431)
 *
 * What comes back now is the deepest cause plus the query's own length, because
 * the two together settle the question the raw echo could not: a parse error
 * reported at a column near the end of the query means it really did arrive
 * truncated, while a column in the middle of a query we sent whole is a genuine
 * syntax error in what the caller wrote.
 */
export function wdqsErrorDetail(body: string, query: string, encodedLength: number): string {
  const transport = encodedLength <= 4000 ? 'GET' : 'POST';
  const shape = `[query ${query.length} chars, ${encodedLength} encoded, sent by ${transport}]`;

  // Deepest cause last — Blazegraph nests ExecutionException -> Malformed
  // QueryException -> ParseException, and the innermost is the precise one.
  const causes = [...body.matchAll(/^(?:Caused by: )?(?:[\w.$]+\.)?(\w*(?:Exception|Error)): (.+)$/gm)];
  const deepest = causes.length ? causes[causes.length - 1] : null;

  if (deepest) {
    const [, name, message] = deepest;
    // "Was expecting one of:" then one token per line. The first few name the
    // token that would have made the query parse, which is the actionable part;
    // the full list runs to 80 entries.
    const expected = [...body.matchAll(/^ {4}(\S.*?) \.\.\.$/gm)].map((m) => m[1]);
    const hint = expected.length
      ? ` Expected one of: ${[...new Set(expected)].slice(0, 6).join(', ')}${expected.length > 6 ? ', ...' : ''}.`
      : '';
    return `${name}: ${message}${hint} ${shape}`;
  }

  // Not a Java trace (an HTML error page, a plain-text refusal). Drop the query
  // echo if it is there, then fall back to the head of what is left.
  const withoutEcho = body.replace(/^SPARQL-QUERY: queryStr=[\s\S]*?\n/, '');
  return `${withoutEcho.trim().slice(0, 200)} ${shape}`;
}
...
/**
 * `sparql` is the declared argument name, but `query` (dbpedia's name for the
 * same concept) is the one a caller moving between the two SPARQL packs
 * guesses. Accept either; name both in the error when neither is given.
 * Fleet #675.
 */
function reqSparql(args: Record<string, unknown>): string {
  const v = (args.sparql ?? args.query) as unknown;
  if (typeof v !== 'string' || !v.trim()) {
    throw new Error(
      'Required argument "sparql" (alias "query") is missing. Pass a string like ' +
        '"SELECT ?p WHERE { ?p wdt:P31 wd:Q5 } LIMIT 5" as either `sparql` or `query`.',
    );
  }
  return v;
}
